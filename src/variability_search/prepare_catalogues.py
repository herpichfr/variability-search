...
class PrepareCatalogues:
    def __init__(self, args, logger=None):
        self.input_dir = args.inputdir
        self.output_dir = args.outputdir if args.outputdir else self.input_dir
    ...
    def organize_data(self, catalogues):
        ...
        if os.path.exists(output_cat_name):
            ...
            self.unified_catalogue = pd.read_csv(output_cat_name)
    ...
    def run(self):
        ...
        if not catalogues:
            ...
            return
        output_catalogue = self.organize_data(catalogues)
        return output_catalogue


if __name__ == "__main__":
    args = parse_args()
    prep_cats = PrepareCatalogues(
        args,
        logger=logger(logfile=args.logfile,
                      loglevel=getattr(logging, args.loglevel))
    )
    output_catalogue = prep_cats.run()
